...
func (q *Queries) FindPosts(ctx context.Context, arg FindPostsParams) ([]Post, error) {
	query := findPosts
	var queryParams []interface{}
	queryParams = append(queryParams, arg.Title)
	if len(arg.Feeds) > 0 {
		for _, v := range arg.Feeds {
			queryParams = append(queryParams, v)
		}
		query = strings.Replace(query, "/*SLICE:feeds*/?", strings.Repeat(",?", len(arg.Feeds))[1:], 1)
	} else {
		query = strings.Replace(query, "/*SLICE:feeds*/?", "NULL", 1)
	}
...
const getPostUrlsByFeed = `-- name: GetPostUrlsByFeed :many
SELECT url
FROM posts
WHERE feed_id = $1
`

func (q *Queries) GetPostUrlsByFeed(ctx context.Context, feedID uuid.UUID) ([]string, error) {
	rows, err := q.db.QueryContext(ctx, getPostUrlsByFeed, feedID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
